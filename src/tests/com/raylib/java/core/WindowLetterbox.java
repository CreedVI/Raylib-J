...
public class WindowLetterbox{
    ...
    private static Vector2 ClampValue(Vector2 value, Vector2 min, Vector2 max) {
        ...
        result.x = Math.min(value.x, max.x);
        result.x = Math.max(result.x, min.x);
        result.y = Math.min(value.y, max.y);
    ...
    public static void main(String[] args){
        ...
        while (!rlj.core.WindowShouldClose())    // Detect window close button or ESC key
        {
            ...
            //SetMouseOffset(-(rlj.core.GetScreenWidth() - (gameScreenWidth*scale))*0.5f, -(rlj.core.GetScreenHeight() - (gameScreenHeight*scale))*0.5f);
            ...
            rlj.textures.DrawTexturePro(target.texture, new Rectangle(0.0f, 0.0f, (float)target.texture.width, (float)-target.texture.height), new Rectangle((rlj.core.GetScreenWidth() - ((float)gameScreenWidth*scale))*0.5f, (rlj.core.GetScreenHeight() - ((float)gameScreenHeight*scale))*0.5f,
