...
public class ColorPalette{
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < MAX_COLORS_COUNT; i++) {
            ...
            colorsRecs[i].y = 80.0f + 100.0f * (i/7) + 10.0f * ((float)i/7);
            // colorsRecs[i].y = 80.0f + 100.0f * (i/7.0f) + 10.0f * (i/7.0f);
        ...
        while (!rlj.core.WindowShouldClose())    // Detect window close button or ESC key
        {
            ...
            // Draw all rectangles
            for (int i = 0; i < MAX_COLORS_COUNT; i++) {
                rlj.shapes.DrawRectangleRec(colorsRecs[i], rlj.textures.Fade(colors[i], colorState[i]? 0.6f : 1.0f));
