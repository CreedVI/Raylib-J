...
public class RLGL {
    ...
    public void rlSetTexture(int id) {
        if (id == 0) {
            ...
            if (GRAPHICS_API_OPENGL_11) {
                ...
                if (rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].textureId != id) {
                    if (rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount > 0) {
                        ...
                        if (rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].mode == RL_LINES) {
                            rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexAlignment = ((rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount < 4) ? rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount : rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount % 4);
                        }
                        else if (rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].mode == RL_TRIANGLES) {
                            rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexAlignment = ((rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount < 4) ? 1 : (4 - (rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount % 4)));
                        }
                        else {
                            rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexAlignment = 0;
                        }

                        if (!rlCheckRenderBatchLimit(rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexAlignment)) {
                            rlglData.getState().vertexCounter += rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexAlignment;
                            rlglData.getCurrentBatch().setDrawCounter(rlglData.getCurrentBatch().getDrawCounter() + 1);
                            rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].mode = rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 2].mode;
                        }
                    }

                    if (rlglData.getCurrentBatch().getDrawCounter() >= DEFAULT_BATCH_DRAWCALLS) {
                        rlDrawRenderBatch(rlglData.getCurrentBatch());
                    }

                    rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].textureId = id;
                    rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].vertexCount = 0;
                }
            }
        }
    }
    ...
    public void rlglInit(int width, int height) {
        ...
        if (RLGL_ENABLE_OPENGL_DEBUG_CONTEXT) {
            context.config.setSupportOpenGLDebug(true);
        }
        ...
        if (GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2) {
            ...
            for (int eye = 0; eye < eyeCount; eye++) {
                ...
                if (rlglData.getState().vertexCounter > 0) {
                    ...
                    for (int i = 0, vertexOffset = 0; i < batch.getDrawCounter(); i++) {
                        ...
                        if ((batch.draws[i].mode == RL_LINES) || (batch.draws[i].mode == RL_TRIANGLES)) {
                            ...
                            if (GRAPHICS_API_OPENGL_33) {
                                ...
                                glDrawElements(GL_TRIANGLES, batch.draws[i].vertexCount / 4 * 6, GL_UNSIGNED_INT, vertexOffset * 6L);
                            }
                            if (GRAPHICS_API_OPENGL_ES2) {
                                glDrawElements(GL_TRIANGLES, batch.draws[i].vertexCount / 4 * 6, GL_UNSIGNED_SHORT, vertexOffset * 6L);
                            }
                        }
    ...
    public boolean rlCheckRenderBatchLimit(int vCount) {
        ...
        if (GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2) {
            if ((rlglData.getState().vertexCounter + vCount) >= (rlglData.getCurrentBatch().vertexBuffer[rlglData.getCurrentBatch().currentBuffer].elementCount * 4)) {
                ...
                int currentMode = rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].mode;
                int currentTexture = rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].textureId;

                rlDrawRenderBatch(rlglData.getCurrentBatch());    // NOTE: Stereo rendering is checked inside

                // Restore state of last batch so new vertices can be added
                rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].mode = currentMode;
                rlglData.getCurrentBatch().draws[rlglData.getCurrentBatch().getDrawCounter() - 1].textureId = currentTexture;
            }
        }
        ...
        if (GRAPHICS_API_OPENGL_11) {
            ...
            switch (vertexAttribType) {
                ...
                case GL_NORMAL_ARRAY:
                    if (buffer != null) {
                        glNormalPointer(GL_FLOAT, 0, buffer.length);
                    }
                    break;
                case GL_COLOR_ARRAY:
                    if (buffer != null) {
                        glColorPointer(4, GL_UNSIGNED_BYTE, 0, buffer.length);
                    }
                    break;
        ...
        if (GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2) {
            ...
            switch (attribType) {
                case SHADER_ATTRIB_FLOAT:
                    if (count == 1) {
                        glVertexAttrib1fv(locIndex, value);
                    }
                    break;
                case SHADER_ATTRIB_VEC2:
                    if (count == 2) {
                        glVertexAttrib2fv(locIndex, value);
                    }
                    break;
                case SHADER_ATTRIB_VEC3:
                    if (count == 3) {
                        glVertexAttrib3fv(locIndex, value);
                    }
                    break;
                case SHADER_ATTRIB_VEC4:
                    if (count == 4) {
                        glVertexAttrib4fv(locIndex, value);
                    }
                    break;
                default:
    ...
    public Matrix rlGetMatrixTransform() {
        ...
        if (GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2) {
            // TODO: Consider possible transform matrices in the rlglData.State.stack
            // Matrix matStackTransform = rlMatrixIdentity();
            // for (int i = rlglData.State.stackCounter; i > 0; i--) matStackTransform = MatrixMultiply(rlglData.State.stack[i], matStackTransform);
